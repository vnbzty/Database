<!DOCTYPE html>
<html>

<head>
<title>Employee Index</title>
</head>

<body>

<form action="welcome.php" method="post">
	<input type="button" onclick="window.location.href='update_department.php'" value="Update department">
	<input type="button" onclick="window.location.href='update_employee.php'" value="Update employee">
	<input type="button" onclick="window.location.href='update_membership.php'" value="Update membership">
	<input type="button" onclick="window.location.href='update_supplier.php'" value="Update supplier">
	<input type="button" onclick="window.location.href='update_catagory.php'" value="Update catagory">
	<input type="button" onclick="window.location.href='update_inventory.php'" value="Update inventory">
</form>

</body>
</html>
