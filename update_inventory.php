...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if ($id) { // update
    ...
    if ($flag == 1) {
      ...
      if ($prize) {
        $query = "UPDATE GOODS SET EMPLOYEE_id = '$prize' WHERE id = '$id'";
        exe_query($con, $query);
      }
      if ($amount) {
        $query = "UPDATE GOODS SET EMPLOYEE_id = '$amount' WHERE id = '$id'";
        exe_query($con, $query);
      }
      if ($employee) {
      ...
      if ($catagory) {
        $query = "UPDATE GOODS SET EMPLOYEE_id = '$catagory' WHERE id = '$id'";
        exe_query($con, $query);
      }
      if ($supplier) {
        $query = "UPDATE GOODS SET EMPLOYEE_id = '$supplier' WHERE id = '$id'";
        exe_query($con, $query);
      }
